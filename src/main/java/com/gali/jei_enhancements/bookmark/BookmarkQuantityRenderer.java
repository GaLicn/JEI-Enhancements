...
public class BookmarkQuantityRenderer {
    ...
    private static void renderSlotQuantity(GuiGraphics guiGraphics, Font font, IngredientListSlot slot, BookmarkManager manager) {
        ...
        // 格式化数量字符串
        String quantityStr = Screen.hasShiftDown()
                ? formatQuantityFull(quantity, isFluidOrChemical)
                : formatQuantity(quantity, isFluidOrChemical);
    ...
    private static String formatQuantityFull(int quantity, boolean isFluid) {
        String suffix = isFluid ? "L" : "";
        return String.format("%,d%s", quantity, suffix);
    }
}
